...
- One `vi.mock()` call at module scope (hoisted):
  - `vi.mock("@/lib/supabase", () => ({ createClient: vi.fn() }))`.
- Build a `makeSaveDeckSupabase({ discardError?, acceptError?, editError? })` factory that distinguishes the three DB steps by the method chain after `.from()`: `.delete()` → discard step, `.update().in()` → accept step, `.update().eq()` → edit step. Each error parameter is `Error | null`.
...
- Boundaries: keep auth, routing, own API routes, and the database real; mock
  only OpenRouter. Because `generateCards()` runs `fetch` server-side
  (`src/lib/services/generation.ts:32`), intercept the app's own
  `/api/generate` with `page.route()` and pass the real response through via
  `route.fetch()`, rewriting only card text so ids stay genuine rows.
