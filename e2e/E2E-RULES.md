...
Stub the app's own `/api/generate` response instead, and pass the real
response through (`route.fetch()`), rewriting only the card text so the ids
remain genuine database rows. See `e2e/generation-persistence.spec.ts`.
